...
void main(){
 ...
 // aC *= texture2D( t_audio , vec2( vUv.y , 0. ) );
  ...
  float lamb = max( 0. , dot( normalize(vLightDir) , vMNorm ));
  float refl = max( 0. , dot( reflect( normalize(vLightDir) , vMNorm  )  , vCamVec ));
 ...
 // gl_FragColor = vec4( vec3( .5 , .4 , .2 ) + vec3( 1. , 1. , .6 ) * aC.xyz * aC1.xyz , 1. ); //aC ; //* vec4(  1000. - vMPos.y , 100. / vMPos.y , .3, 1. );
  ...
  vec3 col =  sem.xyz * hsv(  length(vVel) * .1 * ( 1. + sin(time * .6) ) + time * .2, .8, length( vVel) )  ;//+ (vNorm * .5+ .5) * sem.xyz;
 // col +=  vec3(lamb,lamb,lamb) * sem.xyz * 10. *  hsv(refl*refl*refl* 2., .8, refl*refl*refl )  ;//+ (vNorm * .5+ .5) * sem.xyz;
  
  //col = norm * .5 + .5;
  gl_FragColor = vec4( col , 1.);//pow( vFR, 30. ) * 1. * vAudio + vec4( 0.5 * normalize(vReflection ) + 0.7 , 1. ) *   sem; //vec4( vSEM.x , 0. , vSEM.y , 1. );
